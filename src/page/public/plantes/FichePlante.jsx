    ...
    <div className="space-y-8">
      ...
      <div className="card p-8">
        <div className="flex flex-col md:flex-row items-start gap-6">
          ...
          <div className="flex-1">
            <h1 className="text-3xl md:text-4xl font-serif font-bold italic text-foreground mb-2">
              {plante.nom}
            </h1>
            {plante.nomCommun && (
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4 text-sm text-muted-foreground">
              {plante.famille && (
                <span className="flex items-center gap-1">
                  <BookOpen size={14} /> {plante.famille}
                </span>
              )}
              {plante.origine && (
                <span className="flex items-center gap-1">
                  <MapPin size={14} /> {plante.origine}
                </span>
              )}
            </div>
            {plante.proprietes?.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {plante.proprietes.map((p) => (
                  <Badge key={p} variante="primary">{p}</Badge>
                ))}
              </div>
            )}
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          ...
          <p className="text-foreground/70 leading-relaxed text-sm">
            {plante.usagesPharmaceutiques || plante.usages || "Aucun usage renseigné."}
          </p>
        </div>
      </div>
        ...
        <div className="card p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {plante.composants.map((c) => (
              <span
                key={c}
                className="px-3 py-1.5 bg-secondary rounded-xl text-sm font-medium text-foreground/80 border border-border"
              >
                {c}
              </span>
            ))}
